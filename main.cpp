...
int main()
{
  ...
  try { texts = converter.get_text_documents(); }
  catch(const std::exception& ex1)
  {
    std::cout << "нет доступа к текстам" << std::endl;
    return 1;
  }
  try { responses_limit = converter.get_responses_limit(); }
  catch(const std::exception& ex2)
  {
    std::cout << "нет доступа к лимиту ответов" << std::endl;
  }
  try { requests = converter.get_requests(); }
  ...
  auto res = search_server.search(requests, inverted_index,
				  responses_limit);
  try { converter.put_answers(res); }
  catch(const std::exception& ex4) { std::cout <<
