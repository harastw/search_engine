...
struct SearchServer
{
  std::map<std::string,size_t> to_sort_unique_requests(std::map<std::string,size_t>& unique_requests) const;
  std::map<std::string,size_t> to_unique(std::vector<std::string> requests) const;
  void print_ri(RelativeIndex ri) const;
  void print_res_for_one_request(res_for_one_request res) const;
